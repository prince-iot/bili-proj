...
#define DEVID		"PrinceIot_Device|securemode=2,signmethod=hmacsha1,timestamp=1700892360305|"//产品
...
//					printf("json_value: [%s]\n",json_value->string);
					...
					if(strstr(json_value->valuestring,"led0 on") != NULL)    //控制打开LED0
					{
						...
						AliNet_Publish("/k0hfv4sYShN/PrinceIot_Device/user/update","{\"esp8266\":\"LED_0 ON\"}");  //更新发布LED0状态
...
//						else LED0 = 0;//关灯
					...
					else if(strstr(json_value->valuestring,"led0 off") != NULL)
					{
						...
						AliNet_Publish("/k0hfv4sYShN/PrinceIot_Device/user/update","{\"esp8266\":\"LED_0 OFF\"}");
//						json_value = cJSON_GetObjectItem(json , "value");
...
//						alarm_is_free=0;//上位机控制跟手动控制具有一样的优先级 优先于自动控制，因此要把alarm_is_free 归0
					...
					else if(strstr(json_value->valuestring,"led2 on") != NULL)  
					{
						...
						AliNet_Publish("/k0hfv4sYShN/PrinceIot_Device/user/update","{\"esp8266\":\"LED_1 ON\"}");  
					}
					else if(strstr(json_value->valuestring,"led2 off") != NULL)
					{
				
						AliNet_Publish("/k0hfv4sYShN/PrinceIot_Device/user/update","{\"esp8266\":\"LED_1 OFF\"}");
					}
				
					else if(strstr(json_value->valuestring,"led on") != NULL)
					{
				
						AliNet_Publish("/k0hfv4sYShN/PrinceIot_Device/user/update","{\"esp8266\":\"LED ON\"}");
					}
					else if(strstr(json_value->valuestring,"led off") != NULL)
					{
			
						AliNet_Publish("/k0hfv4sYShN/PrinceIot_Device/user/update","{\"esp8266\":\"LED OFF\"}");
					}
					...
					else if(strstr(json_value->valuestring,"update") != NULL)
					{
						...
						*/
						sprintf(PUB_BUF1,"{\"temp\":%d,\"esp8266\":\"UPDATE OK\"}", 0);      //向缓冲区PUB_BUF1中写入湿度、温度数据
						AliNet_Publish("/k0hfv4sYShN/PrinceIot_Device/user/update", PUB_BUF1);  //向服务器发布缓冲区PUB_BUF1信息，即发布湿度、温度值
					}
